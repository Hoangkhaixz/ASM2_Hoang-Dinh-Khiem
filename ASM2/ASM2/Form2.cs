...
namespace ASM2
{
    public partial class Form2 : Form
    {
        ...
        private void btnprin_Click(object sender, EventArgs e)
        {
            ...
            string price = txttotal.Text; // Giá trước thuế

            // Tính thuế môi trường và VAT
            double priceDouble = double.Parse(price);
            string envTax = (priceDouble / 10).ToString("0.00");
            string vat = ((priceDouble + double.Parse(envTax)) / 10).ToString("0.00");
            string total = (priceDouble + double.Parse(envTax) + double.Parse(vat)).ToString("0.00");

            // Gọi phương thức SetBillInformation trên Form3
            form3.SetBillInformation(name, phone, customerType, waterLastMonth, waterThisMonth, waterUsed, price, envTax, vat, total);
